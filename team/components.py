...
from common.utils import get_integers
...
class VoteView(discord.ui.View):
    ...
    def __init__(self, lang: Lang = Lang.JA) -> None:
    ...
    def set_labels(self, lang: Lang) -> None:
        flag = lang == Lang.JA
    ...
    async def on_error(self, error: Exception, item: Item, interaction: Interaction) -> None:
        ...
        if isinstance(error, RoleNotFound):
            content = {'ja':'ロールが見つかりません。'}.get(interaction.locale,'Role not found.')
        elif isinstance(error, MessageNotFound):
            content = {'ja':'メッセージが見つかりません。'}.get(interaction.locale,'Message not found.')
        elif isinstance(error, NotAuthorized):
            content = {'ja':'この操作をする権限がありません。'}.get(interaction.locale,'You cannot do this interaction.')
        elif isinstance(error, AuthorNotFound):
            content = {'ja':'作者が見つかりません。'}.get(interaction.locale,'Author not found.')
    ...
    async def callback(
        ...
        vote = VoteMessage.convert(interaction.message)
...
class VoteMessage:
    ...
    def embed(self) -> discord.Embed:
        ...
        e.add_field(
            name = '日時' if self.lang == Lang.JA else 'Date',
    ...
    @staticmethod
    def convert(message: MessageLike) -> VoteMessage:
        try:
            if message.embeds:
                pass
            else:
    ...
    async def start(
        ...
        lang = {'ja':Lang.JA}.get(lang, Lang.EN)
        msg = VoteMessage(enemy = enemy, author = interaction.user, role = role, dt = dt, lang = lang)
